...
int main() {
    ...
    *matrix_ptr(m, 2, 2) = -1.0;
     ...
     *matrix_ptr(A, 0, 0) = 0; *matrix_ptr(A, 0, 1) = 0; *matrix_ptr(A, 0, 2) = 1.0;
     *matrix_ptr(A, 1, 0) = 0; *matrix_ptr(A, 1, 1) = 1.0; *matrix_ptr(A, 1, 2) = 0;
     *matrix_ptr(A, 2, 0) = 1.0; *matrix_ptr(A, 2, 1) = 0; *matrix_ptr(A, 2, 2) = 0;
...
// int main() {
//     printf("=== Testing Gaussian elimination solver ===\n\n");
    
//     // Test 1: Ваш тестовый пример (перестановочная матрица)
//     {
//         printf("Test 1: Permutation matrix\n");
//         matrix* A = matrix_alloc(3, 3);
//         *matrix_ptr(A, 0, 0) = 0; *matrix_ptr(A, 0, 1) = 0; *matrix_ptr(A, 0, 2) = 1.0;
//         *matrix_ptr(A, 1, 0) = 0; *matrix_ptr(A, 1, 1) = 1.0; *matrix_ptr(A, 1, 2) = 0;
//         *matrix_ptr(A, 2, 0) = 1.0; *matrix_ptr(A, 2, 1) = 0; *matrix_ptr(A, 2, 2) = 0;
...
//     }
    ...
    // // Test 2: Простая треугольная система
    // {
    //     printf("Test 2: Triangular system\n");
    //     matrix* A = matrix_alloc(3, 3);
    //     *matrix_ptr(A, 0, 0) = 1.0; *matrix_ptr(A, 0, 1) = 2.0; *matrix_ptr(A, 0, 2) = 3.0;
    //     *matrix_ptr(A, 1, 0) = 0.0; *matrix_ptr(A, 1, 1) = 4.0; *matrix_ptr(A, 1, 2) = 5.0;
    //     *matrix_ptr(A, 2, 0) = 0.0; *matrix_ptr(A, 2, 1) = 0.0; *matrix_ptr(A, 2, 2) = 6.0;
        
    //     matrix* B = matrix_alloc(3, 1);
    //     *matrix_ptr(B, 0, 0) = 14.0;
    //     *matrix_ptr(B, 1, 0) = 19.0;
    //     *matrix_ptr(B, 2, 0) = 12.0;
